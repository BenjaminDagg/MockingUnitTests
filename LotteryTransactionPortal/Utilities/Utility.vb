...
Public Class Utility
   ...
   Private Shared mOnes() As String = {"Zero", "One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine", "Ten", "Eleven", "Twelve", "Thirteen", "Fourteen", "Fifteen", "Sixteen", "Seventeen", "Eighteen", "Nineteen"}
   Private Shared mTens() As String = {"Twenty", "Thirty", "Forty", "Fifty", "Sixty", "Seventy", "Eighty", "Ninety"}
   Private Shared mGroups() As String = {"Hundred", "Thousand", "Million", "Billion", "Trillion"}
   ...
   Public Shared Function NumericToText(ByVal aNumber As Int32) As String
      '--------------------------------------------------------------------------------
      ' Returns a number as text.
      '--------------------------------------------------------------------------------
      ' Allocate local vars...
      Dim lGroupDescription As String
      Dim lReturn As String = ""

      Dim lGroup As Integer = 0
      Dim lNumberToProcess As Integer

      If aNumber = 0 Then
         Return mOnes(aNumber)
      End If

      While (aNumber > 0)
         lNumberToProcess = aNumber Mod 1000
         aNumber = aNumber \ 1000

         lGroupDescription = ProcessGroup(lNumberToProcess)
         If Not lGroupDescription Is Nothing Then
            If (lGroup > 0) Then
               lReturn = mGroups(lGroup) & " " & lReturn
            End If
            lReturn = lGroupDescription & " " & lReturn
         End If
   ...
   Private Shared Function ProcessGroup(ByVal Number As Int32) As String
      '--------------------------------------------------------------------------------
      ' Support function for the NumericToText function above.
      '--------------------------------------------------------------------------------
      ' Allocate local vars...
      Dim lOnes As Int32
      Dim lTens As Int32 = Number Mod 100
      Dim lHundreds As Int32 = CType(Number \ 100, Int32)

      Dim lReturn As String = Nothing

      If lHundreds > 0 Then
         lReturn = mOnes(lHundreds) & " " & mGroups(0)
      End If

      If lTens > 0 Then
         If lTens < 20 Then
            If lReturn Is Nothing Then
               lReturn &= mOnes(lTens)
            Else
               lReturn &= " " & mOnes(lTens)
            End If
   ...
   ' Routines for Password encryption/decryption in Startup.vb
