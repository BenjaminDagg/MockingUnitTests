...
Begin VB.Form frm_Payout_MS 
   ...
   Begin VB.CommandButton cmdPromoPoints 
      ...
      Height          =   525
      Left            =   120
      Picture         =   "frm_Payout_MS.frx":0CCA
      Style           =   1  'Graphical
      TabIndex        =   3
   ...
   Begin MSComctlLib.ImageList imgList 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm_Payout_MS.frx":0E14
            Key             =   "PayoutBlack"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm_Payout_MS.frx":437A
            Key             =   "PayoutBlue"
         EndProperty
      EndProperty
   End
   ...
   Begin TabDlg.SSTab sst_Payouts 
      ...
      TabPicture(0)   =   "frm_Payout_MS.frx":78E0
      Tab(0).ControlEnabled=   0   'False
      Tab(0).Control(0)=   "fr_SmartCard"
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "Vouchers"
      TabPicture(1)   =   "frm_Payout_MS.frx":78FC
      ...
      TabPicture(2)   =   "frm_Payout_MS.frx":7918
      Tab(2).ControlEnabled=   0   'False
      Tab(2).Control(0)=   "fr_CashDrawerStatus"
      Tab(2).Control(1)=   "fr_CashDrawerTrans"
      ...
      Begin VB.Frame fr_Vouchers 
         ...
         Begin VB.CommandButton cmd_PayoutVoucher 
            Height          =   645
            Left            =   5040
            Picture         =   "frm_Payout_MS.frx":7934
            Style           =   1  'Graphical
            TabIndex        =   14
         ...
         Begin VB.Image imgLightVchr 
            Height          =   270
            Left            =   120
            Picture         =   "frm_Payout_MS.frx":AE8A
            Top             =   120
            Width           =   270
         End
      ...
      Begin VB.Frame fr_SmartCard 
         ...
         Begin VB.CommandButton cmdPayout 
            Height          =   645
            Left            =   5040
            Picture         =   "frm_Payout_MS.frx":B2BC
            Style           =   1  'Graphical
            TabIndex        =   57
         ...
         Begin VB.Image imgLight 
            Height          =   270
            Left            =   120
            Picture         =   "frm_Payout_MS.frx":E812
            Top             =   120
            Width           =   270
         End
   ...
   Begin VB.Image imgSource 
      ...
      Index           =   3
      Left            =   2835
      Picture         =   "frm_Payout_MS.frx":EC44
      Top             =   180
      Visible         =   0   'False
      Width           =   270
   End
   Begin VB.Image imgSource 
      ...
      Index           =   2
      Left            =   2520
      Picture         =   "frm_Payout_MS.frx":F0A8
      Top             =   180
      Visible         =   0   'False
      Width           =   270
   End
   Begin VB.Image imgSource 
      ...
      Index           =   1
      Left            =   2205
      Picture         =   "frm_Payout_MS.frx":F50A
      Top             =   180
      Visible         =   0   'False
      Width           =   270
   End
   Begin VB.Image imgSource 
      ...
      Index           =   0
      Left            =   1890
      Picture         =   "frm_Payout_MS.frx":F96E
      Top             =   180
      Visible         =   0   'False
      Width           =   270
   End
   ...
   End
End
Attribute VB_Name = "frm_Payout_MS"
...
Dim lcVoucherBalance As Currency
   ...
   ' Clear msReceiptPayoutName and msReceiptPayoutSSN...
   ' msReceiptPayoutName = ""
   ' msReceiptPayoutSSN = ""
   
   ' Store the balance, note that the format function handles any rounding.
   If Len(txt_VoucherAmt.Text) > 0 Then
      lcVoucherBalance = CCur(Format(txt_VoucherAmt.Text, "###,##0.00"))
   End If
   
   ' Set flag to show or hide Name and SSN on receipt
   mbNameAndSSNLabels = (mtVoucher.VoucherType = 1 And mtVoucher.VoucherAmt >= 600#)
   
'   ' If this is a Jackpot Voucher, get name and SSN...
'   If mtVoucher.VoucherType = 1 And mtVoucher.VoucherAmt >= 600# Then
'      ' Set
'      mbShowNameAndSSN = True
'      ' Show the from to collect the name and ssn.
'      frm_ReceiptUserInfo.Show vbModal
'      ' When it closes, msReceiptPayoutName and msReceiptPayoutSSN will be populated.
'   End If
   
   ' Cash Drawer support is not required for DC Lottery.
   '   If mbHasCashDrawer Then
   '      lcDrawerBalance = txt_CurrentBal(1) '  mcCurrentBalVchr
   '      If lcVoucherBalance > lcDrawerBalance Then
   '         lsUserMsg = "The amount to pay exceeds the cash drawer balance." & vbCrLf & _
   '            "Click the 'Add / Remove $' button and add cash to the cash drawer."
   '         MsgBox lsUserMsg, vbExclamation, gMsgTitle
   '         GoTo ExitSub
   ...
   ' Check if VoucherType = Jackpot and the Voucher Amount exceeds the Lockup Amount
   If (mtVoucher.VoucherType = 1) And (lcVoucherBalance > (mcLockupAmt - 0.001)) Then
      If (gSecurityLevel < giSupervisorLevel) Or (InStr(1, "~ADMIN~ADMINISTRATOR~SUPERVISOR~SUPER~", lsPayoutUser, vbTextCompare) > 0) Then
         ' Requires supervisor level (or greater) authorization.
         lsUserMsg = " Payout Amount Exceeds the Lockup Amount." & vbCrLf & _
            "Please have a user with Supervisor level (or greater) permissions authorize this payment."
         MsgBox lsUserMsg, vbInformation, "Authorization Required"
   ...
   With lADOCmd
      ...
      .Parameters.Append .CreateParameter(Name:="@VoucherID", Type:=adInteger, Direction:=adParamInput, Value:=mtVoucher.VoucherID)
      ...
      .Parameters.Append .CreateParameter(Name:="@PayoutAmount", Type:=adInteger, Direction:=adParamInput, Value:=CLng(lcVoucherBalance * 100))
   ...
   If llRC = 0 Then
      ...
      ' Store the Cashier Trans ID as a reference number to print on the voucher payout receipt.
      lsRefNbr = CStr(llCTransID)
      ...
      ' Call print receipt function.
      Call Print_Receipt(lsRefNbr, lcVoucherBalance)
    
      ' Make sure reprint button is visible if allowed.
      cmd_Receipt_Reprint.Visible = mbShowReprint
    
      ' If using cash drawer get Cashiers Summary
      ' DC Lottery does not require Cash Drawer support
      ' If mbHasCashDrawer Then Call GetCashiersSummary
    
      ' Voucher was paid out successfully so set mbValidVoucher to false
      mbValidVoucher = False
      
      ' Record the fact that voucher paid
      Call gConnection.AppEventLog(gUserId, AppEventType.AccountingEvent, "Voucher successfully paid out. ID: " & mtVoucher.VoucherID & " Amount: " & mtVoucher.VoucherAmt & " Barcode: ****" & Right(mtVoucher.BarCode, 4) & ".")
      
      
    
      txt_LastVoucher.Text = mtVoucher.BarCode
      msLastVoucher = mtVoucher.BarCode
...
lsTest = "~" & lsUID & "~"
   ...
   If (Len(lsUID) < 4) Or (Len(lsPWD) < 4) Then
      ...
      MsgBox "The User ID and Password must both contain a minimum of 4 Characters."
      Exit Sub
   Else
...
Dim lcBalance        As Currency
   ...
   If mbPromoPlay Then
      If mcDollarsPerPoint < 0.01 Or miRedeemMultiple < 1 Then
         mbPromoPlay = False
         MsgBox "Promotional Play disabled due to incorrect system settings.", vbExclamation, "Promotional Play Status"
      End If
   End If
...
'--------------------------------------------------------------------------------
   ...
   Else
      ' Display login authorization...
      fr_Vouchers.Visible = False
      fr_Authorize.Visible = True
      txt_UserId.Text = ""
      txt_PssWd.Text = ""
      txt_UserId.SetFocus
   End If
...
'--------------------------------------------------------------------------------
   ...
   ' Hide first and third tabs...
   sst_Payouts.TabVisible(2) = False
...
ErrExit:
   MsgBox "frm_Payout_MS:Unload" & vbCrLf & Err.Description, vbCritical, gMsgTitle
   GoTo ExitSub
...
LocalError:
   MsgBox "frm_Payout_MS::GetAccounts" & vbCrLf & Err.Description, vbCritical, gMsgTitle
   GoTo ExitFunction
...
ErrExit:
   MsgBox "frm_Payout_MS:HandleSSTimeout" & vbCrLf & Err.Description, vbExclamation
   GoTo ExitRoutine
...
ErrExit:
   MsgBox "frm_Payout_MS:Print_CD_AddRemove_Receipt" & vbCrLf & Err.Description, vbCritical, gMsgTitle
   GoTo ExitRoutine
...
ErrExit:
   MsgBox "frm_Payout_MS:GetCashDrawerBalance" & vbCrLf & _
...
Dim lControl      As Control
   ...
   For Each lControl In Controls
      If TypeOf lControl Is TextBox Then
         ...
         If InStr(1, "~txt_StartBal~txt_CashAdded~txt_CashRemoved~txt_TotalPayout~txt_CurrentBal~", lControl.Name, vbTextCompare) = 0 Then
            lControl.Text = ""
         End If
         'lControl.Text = ""
...
Dim lcCDBalance     As Currency
   ...
   Select Case aValidationNbr
      Case 1
         ...
         If (mtVoucher.RedeemedState = False) And ((mtVoucher.CreateDate + miVoucherExpDays) < Now()) Then
            txt_Result.Text = "Voucher " & mtVoucher.FormattedBarcode & vbCrLf & "has expired. Call Supervisor."
            txt_VoucherCode.Text = ""
      ...
      Case 6
         ...
         If mtVoucher.UcvTransferred = True Then
            txt_Result.Text = "Voucher " & mtVoucher.FormattedBarcode & vbCrLf & _
                              "expired on " & Format(mtVoucher.UcvTransferDate, "mm/dd/yyyy") & "."
            txt_VoucherCode.Text = ""
            VoucherValidation = False
         End If
...
LocalError:
   ...
   MsgBox "frm_Payout_MS::VoucherValidation error: " & lsError & vbCrLf & Err.Description, vbCritical, gMsgTitle
   GoTo ExitFunction
...
Dim lsValue       As String
   ...
   lsSQL = "SELECT VOUCHER_ID, VOUCHER_TYPE, BARCODE, " & _
           ...
           "IS_VALID FROM VOUCHER WHERE BARCODE = '%s'"
   lsSQL = Replace(lsSQL, SR_STD, lsBarcode, 1, 1)
...
LocalError:
   MsgBox "frm_Payout_MS::GetVoucherInfo" & vbCrLf & Err.Description, vbCritical, gMsgTitle
   GoTo ExitFunction
...
LocalError:
   MsgBox "frm_Payout_MS::LoadVouchers" & vbCrLf & Err.Description, vbCritical, gMsgTitle
   GoTo ExitFunction
