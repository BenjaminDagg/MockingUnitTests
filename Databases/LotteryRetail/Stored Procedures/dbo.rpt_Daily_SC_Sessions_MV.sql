...
--------------------------------------------------------------------------------
*/
CREATE PROCEDURE [dbo].[rpt_Daily_SC_Sessions_MV] @StartDate DateTime, @EndDate DateTime
AS
...
FROM CASHIER_TRANS csht
   ...
   JOIN VOUCHER_TRANS_DETAILS D ON V.VOUCHER_ID = D.VOUCHER_ID
   JOIN VOUCHER_TRANS T ON D.VOUCHER_TRANS_ID = T.VOUCHER_TRANS_ID
WHERE
