...
/*
--------------------------------------------------------------------------------
Procedure: rpt_WeeklyRevByMachineRP user stored procedure.

Created: 03-15-2011 by Terry Watkins

Purpose: Returns summarized data for the Weekly Revenue by Machine report that is
...
--------------------------------------------------------------------------------
*/
CREATE PROCEDURE [dbo].[rpt_WeeklyRevByMachineRP] @Acct_Date DateTime
AS
...
WHILE (@@FETCH_STATUS = 0)
   BEGIN
      ...
      WHERE
         ...
         dbo.ufnGetAcctDateFromDate(v.REDEEMED_DATE) BETWEEN @Acct_Date AND @EndAcctDate
      
      -- Update VoucherPaidTotal column in the temp table.
...
-- Close and Deallocate the RevByMachine Cursor...
CLOSE RevByMachine
