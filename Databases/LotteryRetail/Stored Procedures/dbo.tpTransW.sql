...
/*
--------------------------------------------------------------------------------
User Stored Procedure tpTransW

Created 04-11-2003 by Chris Coddington

Desc: Handles all winning transactions.
...
Parameters:
   ...
   @CoinsWon            Number of Coins that were won (or cents won if Keno)
...
Terry Watkins 01-18-2006     v5.0.2
  ...
  Added logic to NOT set CARD_ACCT.STATUS = 2 for an MAGAM Jackpot (TPI_ID=2).
...
A. Murthy 07-25-2007         v6.0.1
  For Bingo set @FormMultiplier = @CoinsBet and do not call stored procedure
  tpDeactivateTicket.
...
Terry Watkins 01-24-2008     v6.0.2
  Change the datatype of @ProgressivePct from Int to Decimal(6,4).
...
SET @ErrorID            = 0
SET @ErrorOverride      = 0
SET @ErrorSource        = 'tpTransW Stored Procedure'
SET @ErrorText          = ''
SET @ErrorTypeID        = 0
SET @EventCode          = 'SP'
...
-- Check Debug mode...
SELECT @Debug = ISNULL(DEBUG_MODE, 0) FROM DEBUG_SETTING WHERE DEBUG_ENTITY = 'tpTransW'

-- If Debug mode, save argument values...
IF (@Debug = 1)
   -- Record argument values.
   BEGIN
      SET @MsgText = 'tpTransW Argument Values:  Barcode: ' + @Barcode +
         '  CardAccount: '         + @CardAccount +
         '  CoinsBet: '            + CAST(@CoinsBet AS VarChar) +
         '  CoinsWon: '            + CAST(@CoinsWon AS VarChar) +
         '  DealNumber: '          + CAST(@DealNumber AS VarChar) +
         '  LinesBet: '            + CAST(@LinesBet AS VarChar) +
         ...
         '  TicketNumber: '        + CAST(@TicketNumber AS VarChar) +
         '  TierLevel: '           + CAST(@TierLevel AS VarChar) +
         '  PressUpCount: '        + CAST(@PressUpCount AS VarChar) +
...
GROUP BY PROGRESSIVE_TYPE_ID, DENOMINATION
      ...
      IF (@IsPromoOn = 1) SET @PromoAmount = @TabCost
...
IF (@Debug = 1)
   -- Record argument and calculated values.
   BEGIN
      SET @MsgText = 'tpTransW Calculated values:  BaseTrans: '
      ...
      IF ((@MachinePromoBal - @NewSystemPromoBal) <= @MaxBalAdjust)
         BEGIN
            ...
            EXEC tpAdjustMachinePromoBalance
               @MachineNumber, @CardRequired, @MachinePromoBal, @NewSystemPromoBal, 'tpTransW',
               @TimeStamp, @CasinoID, @AcctDate, @MachGameCode, @LocationID

            IF (@Debug = 1)
               BEGIN
                  SET @MsgText = 'tpTransW executed tpAdjustMachinePromoBalance: MachinePromoBalance = ' +
      ...
      ELSE
         BEGIN
            ...
            IF (@Debug = 1)
               BEGIN
                  SET @MsgText =
                     'tpTransW Promo Balance difference exceeds adjustment threshold: ' +
...
IF (@ErrorID = 0 AND @CardRequired = 1)
   BEGIN
      ...
      -- Note that for Cardless play, we already have the balance from the machine (line 266)
      SELECT
         @MachNo  = MACH_NO,
         @Balance = BALANCE
      FROM CARD_ACCT
...
IF (@ErrorID = 0)
   BEGIN
      ...
      IF (@Debug = 1)
         BEGIN
            SET @MsgText =
               'tpTransW Deal/Form/GameTypeInfo -  IsActive: '
               ...
               '  MaxCoinsBet: '        + CAST(@MaxCoinsBet AS VarChar) +
               '  PayscaleName: '       + ISNULL(@PayscaleName, '<null>') +
               '  TabsPerSubset: '      + ISNULL(CAST(@TabsPerSubset AS VarChar), '<null>') +
               '  ProgressiveAmt: '     + CAST(@ProgressiveAmt AS VarChar)
            
            EXEC InsertDebugInfo @TransID, @MsgText, @MachNbrAsInt
         END
      
      -- Did we get a row back from Deal, Form, GameType, Payscale retrieval above?
      IF (@IntValue > 0)
         BEGIN
                  ...
                  IF (@IsActive = 1)
                     ...
                     BEGIN
                        ...
                        IF (@Debug = 1)
                           BEGIN
                              SET @MsgText =
                                 'tpTransW - TierCoinsWon: ' + CAST(@TierCoinsWon AS VarChar) +
                        ...
                        IF (@TierCoinsWon = @CoinsWon)
                           BEGIN
                              ...
                              IF (@CardRequired = 1)
                                 -- In Card Required mode, update Balance on the CARD_ACCT table.
                                 BEGIN
                                    UPDATE CARD_ACCT SET
                                       PROMO_AMOUNT = PROMO_AMOUNT + @PromoAmount,
                                       BALANCE      = BALANCE + @NetTrans + @ProgressiveAmt,
                                       @Balance     = BALANCE + @NetTrans + @ProgressiveAmt
                                    WHERE CARD_ACCT_NO = @CardAccount
                                 ...
                                 END
                              ELSE
                                 -- For Cardless Play mode, update Balance on the MACH_SETUP table.
                                 BEGIN
                              ...
                              -- Is the Win Amount > lockup amount and in card required mode and not interfacing with MGAM?
                              IF ((@CardRequired = 1) AND (@CTTransAmount >= @LockupAmount) AND (@TpiID <> 2))
                                 BEGIN
                                    -- Deactivate the card.
                        ...
                        ELSE
                           BEGIN
                              SET @ErrorID = 113
                              SET @SaveTrans = 1
                              SET @ErrorText = ',Tier submitted: ' + 
                                     CONVERT(varchar(16), @TierLevel) + ' is not valid. TierCoinsWon = ' +
                                     CONVERT(varchar(16), @TierCoinsWon) + ' MachineCoinsWon = ' + 
                                     CONVERT(varchar(16), @CoinsWon)
                           END
                     END
                  ELSE
                     -- Deal is not active.
      ...
      IF (@@ROWCOUNT = 0)
         BEGIN
            INSERT INTO DEAL_STATS
               (DEAL_NO, PLAY_COUNT, WIN_COUNT, AMOUNT_PLAYED, BASE_AMOUNT_WON, TOTAL_WIN_AMOUNT, PROG_CONTRIBUTION, LAST_PLAY, FIRST_PLAY)
            VALUES
               (@DealNo, 1, 1, @TabCost, @BaseTrans, @CTTransAmount, @ProgContribution, @TimeStamp, @TimeStamp)
         END
...
@ProgContributionPass = @ProgContribution
      ...
      IF (@MultiBetDeals = 0)
         BEGIN
            ...
            IF (@@ROWCOUNT = 0)
               BEGIN
                  -- Retrieve the total number of winners for this Tier.
...
IF (@SaveTrans >= 1)
   ...
   BEGIN
      ...
      EXEC @CasinoTransID = tpInsertCasinoTrans
         @CasinoID, @DealNo, @RollNo, @TicketNo, @MachineDenom, @CTTransAmount, @Barcode,
      ...
      IF ((@MachineBal - @Balance) <= @MaxBalAdjust)
         BEGIN
            ...
            EXEC tpAdjustMachineBalance
               @MachineNumber, @CardRequired, @MachineBal, @Balance, 'tpTransW',
      ...
      ELSE
         ...
         BEGIN
            ...
            IF (@Debug = 1)
               BEGIN
                  SET @MsgText =
                     'tpTransW Balance difference exceeds adjustment threshold: ' +
...
IF (@SaveTrans >= 2) AND (@CTTransAmount >= @LockupAmount)
   BEGIN
...
IF (@Debug = 1)
   BEGIN
      SET @MsgText =
         'tpTransW Return Values - ErrorID: ' + CAST(@ErrorID AS VarChar(10)) +
