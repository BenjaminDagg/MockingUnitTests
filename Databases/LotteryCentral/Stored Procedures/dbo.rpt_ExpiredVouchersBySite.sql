...
/*
--------------------------------------------------------------------------------
Procedure: rpt_ExpiredVouchersBySite user stored procedure.

Created: 02-21-2012 by Aldo Zamora

Purpose: Returns data for the Expired Vouchers by Site report.
...
CREATE PROCEDURE [dbo].[rpt_ExpiredVouchersBySite]
   @StartDate DATETIME
  ,@EndDate DATETIME
...
WHERE
   REDEEMED_STATE = 0
   AND LOCATION_ID IN (SELECT Val FROM dbo.ufn_StringToTable(@LocationID, ',', 1))
