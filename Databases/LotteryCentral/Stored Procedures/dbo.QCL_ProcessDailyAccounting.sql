...
/*
--------------------------------------------------------------------------------
Procedure: QCL_ProcessDailyAccounting user stored procedure.
...
data and store in QCL_DailyAccounting.
...
--------------------------------------------------------------------------------
*/
CREATE PROCEDURE [dbo].[QCL_ProcessDailyAccounting] 
...
SELECT @EventTypeID = EventTypeId FROM dbo.EventType WHERE EventName = 'QCLLotteryAccounting'
...
BEGIN TRY    
   INSERT INTO @Location
      ...
      FROM dbo.JobStatus
      WHERE
         JobName = 'QCL_InsertDailyAccountingData'
         AND Success = 1
   ...
   WHILE @RowID > 0
      BEGIN
         ...
         EXEC dbo.QCL_RetrieveRemoteDailyAccountingData
...
BEGIN CATCH
   ...
   VALUES
      (@EpochDays, @Date, @EventTypeID, 'SQL Error Number: ' + @ErrorID,  @ErrorDescription, 'QCL_ProcessDailyAccounting', -1)
END CATCH
